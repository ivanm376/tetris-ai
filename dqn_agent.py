...
class DQNAgent:
    ...
    def __init__(
        ...
        self.memory_file = shelve.open("memory001", writeback=True)

        if not os.path.isfile("model001"):
            print("INITIALIZING MODEL")
            self.model = self._build_model()
        else:
            self.model = load_model("model001")

        if not os.path.isfile("memory001.db"):
            print("INITIALIZING MEMORY")
            self.memory_file["memory"] = deque(maxlen=mem_size)  # init memory

        self.memory = self.memory_file["memory"]

    def signal_handler(self, sig, frame):
        print("Saving memory/model !!")
        save_model(self.model, "model001")
